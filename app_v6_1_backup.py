...
def view_project_workspace() -> None:
    ...
    with t1:
        show_info = st.toggle(
            ...
            key=f"toggle_show_project_{project['id']}",
        )
        st.session_state[expand_key] = show_info
    with t2:
        edit_mode = st.toggle(
            "Modo edición",
            value=st.session_state[edit_key],
            key=f"toggle_edit_project_{project['id']}",
            disabled=not st.session_state[expand_key],
        )
        st.session_state[edit_key] = edit_mode if st.session_state[expand_key] else False

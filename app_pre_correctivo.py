...
def new_task_view():
    """
    ESTADO B: Nueva tarea — fullscreen, simple, una acción principal
    - Inputs: Título, Descripción, Proyecto
    - Acción principal: "Ver propuesta"
    - Sin pasos numerados, sin confusiones
    """
    st.session_state["view"] = "new_task"

    projects = get_projects()
    if not projects:
        st.error("⚠️ Necesitas crear un proyecto primero. Ve a Home.")
        if st.button("Volver a Home", key="new_task_no_projects"):
            st.session_state["view"] = "home"
            st.rerun()
        return
    ...
    task_title = st.text_input(
        "Título",
        placeholder="Ej: Resumir documento, Escribir email, Analizar datos...",
        key="new_task_title"
    )
    ...
    task_description = st.text_area(
        ...
        placeholder="Detalles adicionales, contexto, restricciones...",
        height=100,
    ...
    with col1:
        if st.button("Ver propuesta", key="new_task_view_proposal", use_container_width=True, type="primary",
                     disabled=not task_title.strip()):
            if task_title.strip() and selected_project:
                # Crear tarea en estado "borrador"
                tid = create_task(
                    selected_project["id"],
                    task_title.strip(),
    ...
    with col2:
        if st.button("Cancelar", key="new_task_cancel", use_container_width=True):
            st.session_state["view"] = "home"
            st.rerun()
...
def proposal_view():
    """
    ESTADO: Propuesta — muestra decisión del Router
    Estructura fija: Modo | Modelo | Por qué
    """
    st.session_state["view"] = "proposal"
    ...
    if not task_id or not project_id:
        st.error("Tarea o proyecto no encontrado")
        if st.button("Volver a Home", key="proposal_back_error"):
            st.session_state["view"] = "home"
            st.rerun()
        return
    ...
    with get_conn() as conn:
        task = conn.execute("SELECT title, description FROM tasks WHERE id = ?", (task_id,)).fetchone()
    ...
    with col1:
        st.markdown("**Modo**")
        st.markdown("ECO")  # Router decide en Hito B

    with col2:
        st.markdown("**Modelo**")
        st.markdown("Gemini 2.5 Flash Lite")  # Router decide en Hito B

    with col3:
        st.markdown("**Por qué**")
        st.markdown("Tarea clara, prioridad rapidez")  # Router decide en Hito B
    ...
    with col1:
        if st.button("Ejecutar", key="proposal_execute", use_container_width=True, type="primary"):
            # Aquí irá la lógica de ejecución en Hito B
            st.info("✓ Ejecución (disponible en Hito B)")
            st.session_state["view"] = "result"
            st.rerun()

    with col2:
        if st.button("Cancelar", key="proposal_cancel", use_container_width=True):
            st.session_state["view"] = "home"
...
def render_header_minimal():
    """
    Header mínimo: Logo + Breadcrumb (sin sidebar)
    """
    ...
    with col2:
        ...
        if active_project_id:
            with get_conn() as conn:
                ...
                if project:
                    breadcrumb_parts.append(project['name'][:30])

        if active_task_id:
            with get_conn() as conn:
                task = conn.execute("SELECT title FROM tasks WHERE id = ?", (active_task_id,)).fetchone()
                if task:
                    breadcrumb_parts.append(task['title'][:30])
        ...
        if current_view == "radar":
            ...
            breadcrumb_parts.append("[Proyecto]")
        else:

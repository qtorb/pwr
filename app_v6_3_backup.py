...
def view_project_workspace() -> None:
    ...
    with top_a:
        if st.button(
            "Ocultar ficha del proyecto" if st.session_state[expand_key] else "Mostrar ficha del proyecto",
            key=f"project_visibility_button_{project['id']}",
    ...
    if st.session_state[expand_key]:
        with top_b:
            st.session_state[edit_key] = st.toggle(
                "Editar ficha del proyecto",
                value=st.session_state[edit_key],
                key=f"project_edit_toggle_{project['id']}",
            )

...
def view_project_workspace() -> None:
    ...
    with st.expander("Información del proyecto", expanded=False):
        if not st.session_state["edit_project_mode"]:
            ...
            description = st.text_area("Descripción breve", value=project["description"], height=90)
            objective = st.text_area("Objetivo del proyecto", value=project["objective"], height=90)
            base_context = st.text_area("Contexto de referencia del proyecto", value=project["base_context"], height=160)
            base_instructions = st.text_area("Reglas estables del proyecto", value=project["base_instructions"], height=140)

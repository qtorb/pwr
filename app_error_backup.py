...
def create_task(project_id: int, title: str, description: str, task_type: str, context: str, uploaded_files) -> int:
    ...
    with get_conn() as conn:
        ...
        conn.execute(
            """
            UPDATE tasks
            SET uploads_json = ?, updated_at = ?
            WHERE id = ?
            """
            ,
            (json.dumps(task_files, ensure_ascii=False), now_iso(), tid),
        )
        return tid
...
def save_execution_result(task_id: int, model_used: str, router_summary: str, llm_output: str, useful_extract: str) -> None:
    with get_conn() as conn:
        conn.execute(
            """
            UPDATE tasks
            SET suggested_model = ?, router_summary = ?, llm_output = ?, useful_extract = ?,
                status = 'ejecutado', updated_at = ?
            WHERE id = ?
            """
            ,
            (model_used, router_summary, llm_output, useful_extract, now_iso(), task_id),
        )
    ...
    with c1:
        st.markdown(
            ...
            <div class="card">
                ...
                <div class="muted">Selecciona un proyecto, captura una tarea, ejecútala con el Router y conviértela en activo reusable.</div>
            </div>
            """
            ,
            unsafe_allow_html=True,
...
def project_view():
    ...
    with right:
        ...
        st.markdown(f"**{task['suggested_model'] or 'Pendiente de ejecución'}**")
        st.text(task["router_summary"] or "Todavía no se ha ejecutado el Router.")
        ...
        if st.button("Ejecutar con Router", use_container_width=True):
            with st.spinner("El Router está analizando la tarea y ejecutando el modo más adecuado..."):
                result = execution_service.execute(task_input)
                output = result.output_text
                extract = output[:700]

                router_summary = (
                    f"Modelo recomendado: {result.metrics.model_used}\n"
                    f"Modo: {result.routing.mode}\n"
                    f"Nivel de complejidad: {result.routing.complexity_score:.2f}\n\n"
...
"""
        ...
        if trace:
            with st.expander("Ver trazabilidad"):
                ...
                st.write(f"**Coste estimado:** {trace['estimated_cost']}")
                st.write("**Motivo de la decisión:**")
                st.write(trace["reasoning_path"])

        st.markdown("### Resultado")
        output = st.text_area("Pega aquí el resultado del modelo o revisa el generado por el Router", value=task["llm_output"], height=200)
